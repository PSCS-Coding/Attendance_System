        <div id="navigation">
            <h1 class="header">Admin View</h1>
            <a href="../../" class="option">Back to Main Page</a>
            <a href="u_students.php" class="option">Students</a>
            <a href="u_facilitators.php" class="option">Facilitators</a>
            <a href="u_allottedhours.php" class="option">Allotted Hours</a>
            <a href="u_passwords.php" class="option">Passwords</a>
            <a href="u_holidays.php" class="option">Holidays</a>
            <a href="u_globals.php" class="option">Globals</a>
            <a href="edit_events.php" class="option">Events</a>
            <a href="u_groups.php" class="option">Groups</a>
            <a href="offsite_stats.php" class="option">Offsite Stats</a>
            <a href="miscellaneous.php" class="option">Miscellaneous</a>
            <a href="example.php" class="option disabled">IN DEVELOPMENT</a>
                    </div>
