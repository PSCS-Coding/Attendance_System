...
// If user has selected a student from the dropdown, load the events for that student (using GET variable)
...
if (!empty($_GET['eventid'])) { // This conditional will fire if a user has clicked the "save" or "delete" button for a row
    $eventid = $_GET['eventid'];
    $changerow = "inline_edit_" . $_GET['eventid'];
    $deleterow = "inline_delete_" . $_GET['eventid'];
    if (!empty($_POST['edit_submit'])) { // For saving an edited event
        if (!empty($_POST['status_select'])) { // update the status of the event
        ...
        if (!empty($_POST['info_edit'])) { // update the info of the event
        ...
        if (!empty($_POST['stamp_edit'])) { // update the timestamp of the event
            $stamp = $_POST['stamp_edit'];
    ...
    if (!empty($_POST[$deleterow])) { // Deletes an event for which the "delete" button was pressed
...
if (!empty($_POST['new_submit'])) { // handles adding a brand new event
   if (!empty($_POST['new_timestamp']) && !empty($_POST['new_status_id'])) { // timestamp and status are required
...
// generate array of current students
...
// keeps student id in GET variable
if (!empty($_GET['id'])) {
...
<body class="edit-events">
	...
	<?php
	// Feedback message when an event is deleted
	...
	<div id="TopHeader" class="">
		...
		<div class="centerr">
			<form method='post' id='studentform' class='studentselect' action='<?php echo basename($_SERVER['PHP_SELF']); ?>'>
				...
				<?php
				foreach($current_users_result as $student) { // student picker
		...
		?>
					...
					<?php } else { ?>
						<tr class="<?php echo $event['statusname'] ?>">
							...
							<td><?php if ($event['statusname'] == 'Offsite' || $event['statusname'] == 'Field Trip' || $event['statusname'] == 'Late') {
