...
if(isset($_POST['Submit']))
{
	...
	if($_SESSION['set'] == 1 || $_SESSION['adminSet'] == 1)
		...
		die("Wrong password :^) <br /><form method='get' action='index.php'><button type='submit'>Retry</button></form>");
...
<?php
$urlholiday = "holidaypage.php";
$holidayresult= $db_server->query("SELECT date FROM holidays ORDER BY date");
$currtoday = date("Y-n-j");
while ($list = mysqli_fetch_assoc($holidayresult)) {
$currholiday = $list['date'];

if ($currtoday == $currholiday) {
    
    header("location:$urlholiday");
